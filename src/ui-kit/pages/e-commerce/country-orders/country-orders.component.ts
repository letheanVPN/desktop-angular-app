...
@Component({
  selector: 'lthn-ui-country-orders',
  styleUrls: ['./country-orders.component.scss'],
  template: `
    <nb-card [size]="breakpoint.width >= breakpoints.md ? 'medium' : 'giant'">
      <nb-card-header>Country Orders Statistics</nb-card-header>
      <nb-card-body>
        <lthn-ui-country-orders-map (selectEvent)="selectCountryById($event)"
                                countryId="USA">
        </lthn-ui-country-orders-map>
        <lthn-ui-country-orders-chart [countryName]="countryName"
                                  [data]="countryData"
                                  [labels]="countriesCategories"
                                  maxValue="20">
        </lthn-ui-country-orders-chart>
      </nb-card-body>
    </nb-card>
  `,
})
